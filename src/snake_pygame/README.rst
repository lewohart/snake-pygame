...
The boilerplate code provides a dummy ``main`` function that prints out the word 'Hello'::
    ...
    >> from snakepygame import main
    >> main()
    
When the package is installed via ``easy_install`` or ``pip`` this function will be bound to the ``snake_pygame`` executable in the Python installation's ``bin`` directory (on Windows - the ``Scripts`` directory).
